...
for i in $TARGETARCH; do
	...
	make clean
	$initpwd/configure  --with-protocols="bfd bgp pipe static" --enable-client=yes --sysconfdir="/etc/bird/" --with-runtimedir="/usr/local/var/run/" --localstatedir="/usr/local/var/" --enable-pthreads=yes -with-sysconfig=linux --build=$ARCH --host=$HOSTARCH
	make
	rm bird birdcl
